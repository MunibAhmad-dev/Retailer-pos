...
// Dynamic path for Retail POS in AppData (try multiple paths)
let dbPath = path.join(process.env.APPDATA, 'retail-pos', 'pos.db');

if (!fs.existsSync(dbPath)) {
    dbPath = path.join(process.env.APPDATA, 'Retail POS', 'pos.db');
}

if (!fs.existsSync(dbPath)) {
    dbPath = path.join(process.env.APPDATA, 'RetailPOS', 'pos.db');
}

if (!fs.existsSync(dbPath)) {
    console.error('Database file not found at any of:');
    console.error('- ' + path.join(process.env.APPDATA, 'retail-pos', 'pos.db'));
    console.error('- ' + path.join(process.env.APPDATA, 'Retail POS', 'pos.db'));
    console.error('- ' + path.join(process.env.APPDATA, 'RetailPOS', 'pos.db'));
    console.error('\nPlease open the app first to initialize the database.');
    process.exit(1);
}
...
console.log('Seeding dashboard data with 1000 transactions (200/month for 5 months) and 200+ inventory items...\n');
...
// Get or create existing product IDs
function getExistingProducts() {
    try {
        const stmt = db.prepare('SELECT id, price, purchase_price FROM products LIMIT 300');
        return stmt.all();
    } catch (e) {
        return [];
    }
...
// Get or create existing customer IDs
function getExistingCustomers() {
    try {
        const stmt = db.prepare('SELECT id FROM customers LIMIT 150');
        return stmt.all().map(c => c.id);
    } catch (e) {
        return [];
    }
}

// 1. Seed 200+ Products for Inventory
console.log('Seeding 220 products for inventory...');
const insertProduct = db.prepare('INSERT OR IGNORE INTO products (name, price, purchase_price, stock, barcode, category, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, datetime("now", "localtime"), datetime("now", "localtime"))');
const productIds = [];
...
for (let i = 0; i < 220; i++) {
    ...
    const retailPrice = Math.floor(purchasePrice * (1.3 + Math.random() * 0.4)); // 30-70% markup
...
// 2. Ensure minimum customers exist
console.log('Ensuring 50+ customers exist...');
const insertCustomer = db.prepare('INSERT OR IGNORE INTO customers (name, phone, email, address, created_at, updated_at) VALUES (?, ?, ?, ?, datetime("now", "localtime"), datetime("now", "localtime"))');
const customerIds = [];
...
// 3. Seed 1000 Sales Transactions (200 per month for 5 months)
console.log('Seeding 1000 sales transactions across 5 months (200 per month)...');
...
// Generate transactions for 5 months (150 days back)
for (let month = 0; month < 5; month++) {
    ...
    console.log(`  Month ${month + 1}: ${monthStart.toLocaleDateString()} to ${monthEnd.toLocaleDateString()}`);
    ...
    for (let transNum = 0; transNum < 200; transNum++) {
        ...
        const taxAmount = Math.floor((subtotal - discount) * 0.17); // 17% tax
        ...
        try {
            ...
            console.error('Error inserting sale:', e.message);
        }
    }
}
...
// 4. Seed Customer Payments (Qaraz)
...
console.log('Summary:');
console.log(`- 220+ Products (inventory items)`);
console.log(`- 1000 Sales transactions`);
console.log(`- 200 transactions per month for 5 months`);
