...
export default function BalanceSheet() {
   ...
   const [activityFilter, setActivityFilter] = useState<'' | 'sale' | 'purchase' | 'expense'>('');
   ...
   const fetchZakatData = async () => {
      ...
      try {
         const startDate = `${zakatFrom} 00:00:00`;
         const endDate = `${zakatTo} 23:59:59`;
         const res = await window.api.getBalanceSheet({ startDate, endDate });
         if (res.success) {
            setZakatData(res.data);
         }
      } catch (error) {
   ...
   const filteredTransactions = useMemo(() => {
      ...
      return (data.recentTransactions || []).filter(txn => {
         ...
         const matchesFilter = activityFilter === '' || txn.type === activityFilter;
   ...
   // Use either local zakatData (if filtered) or main data
   const zSource = zakatData || data;
   const zakatRate = Number(zSource.zakatRate ?? 0.025);
   const zakatGross = Number(zSource.zakatableAssetsGross ?? (zSource.inventoryValue + zSource.receivables));
   const zakatNet = Number(zSource.zakatableAssetsNet ?? Math.max(0, zakatGross - zSource.payables));
         ...
         <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 print:hidden">
            ...
            <div className="flex flex-wrap items-center gap-2 bg-muted/30 p-1 rounded-lg border border-border/50">
               ...
               <Button
                  variant="outline"
                  size="sm"
                  className="h-8 gap-2 border-emerald-300 text-emerald-700 hover:bg-emerald-50"
               ...
               <CardContent>
                  <div className="space-y-4">
                     ...
                     {data.paymentStats.length === 0 && (
                        <div className="text-center py-8 text-muted-foreground text-sm italic">No payment data for this period</div>
                     )}
                  </div>
               </CardContent>
            </Card>
            ...
            <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-background/80 backdrop-blur-sm animate-in fade-in">
               <Card className="w-full max-w-lg shadow-2xl border-emerald-500/20 overflow-hidden">
                  <CardHeader className="bg-emerald-500/10 border-b relative">
                     ...
                     <CardDescription className="text-emerald-900/60 font-medium">Select period to calculate Zakat (2.5% of net wealth)</CardDescription>
                  </CardHeader>
                  <CardContent className="space-y-6 pt-6">
                     <div className="flex flex-col gap-4 p-4 rounded-xl bg-emerald-50/50 border border-emerald-100">
                        <div className="flex items-center justify-between">
                           <span className="text-xs font-bold uppercase tracking-widest text-emerald-800">Calculation Period</span>
                           {zakatLoading && <Loader2 className="animate-spin text-emerald-600" size={16} />}
                        </div>
                        <div className="grid grid-cols-2 gap-3">
                           <div className="space-y-1">
                              <label className="text-[10px] font-bold text-emerald-700 uppercase">From</label>
                              <Input type="date" value={zakatFrom} onChange={(e) => setZakatFrom(e.target.value)} className="h-9 text-xs border-emerald-200 focus:border-emerald-500 bg-white" />
                           </div>
                           <div className="space-y-1">
                              <label className="text-[10px] font-bold text-emerald-700 uppercase">To</label>
                              <Input type="date" value={zakatTo} onChange={(e) => setZakatTo(e.target.value)} className="h-9 text-xs border-emerald-200 focus:border-emerald-500 bg-white" />
                           </div>
                        </div>
                        <Button onClick={fetchZakatData} disabled={zakatLoading} className="w-full bg-emerald-600 hover:bg-emerald-700 h-9 font-bold text-xs gap-2">
                           <Activity size={14} /> Update Calculation
                        </Button>
                     </div>

                     <div className="flex items-center justify-between rounded-xl border border-emerald-200 bg-emerald-50/40 p-3">
                        <span className="text-xs font-bold uppercase tracking-widest text-emerald-900">Eligibility Status</span>
                        <Badge className={cn("font-black", zakatNet >= nisab ? "bg-emerald-600 hover:bg-emerald-700" : "bg-red-600 hover:bg-red-700")}>
                     ...
                     <div className="grid grid-cols-1 gap-4">
                        ...
                        <div className="space-y-3">
                           <div className="flex justify-between text-sm py-2 border-b border-emerald-100">
                              <span className="text-muted-foreground flex items-center gap-2"><Package size={14} /> Stock Value (at end)</span>
                              <span className="font-bold">{fmt(zSource.inventoryValue)}</span>
                           </div>
                           <div className="flex justify-between text-sm py-2 border-b border-emerald-100">
                              <span className="text-muted-foreground flex items-center gap-2"><Users size={14} /> Total Receivables</span>
                              <span className="font-bold">{fmt(zSource.receivables)}</span>
                           </div>
                           <div className="flex justify-between text-sm py-2 border-b border-emerald-100">
                              <span className="text-muted-foreground flex items-center gap-2"><Truck size={14} /> Less: Payables</span>
                              <span className="font-bold text-red-600">- {fmt(zSource.payables)}</span>
                           </div>
                           <div className="flex justify-between text-sm py-2 border-b-2 border-emerald-200">
                              <span className="text-emerald-800 font-bold flex items-center gap-2"><TrendingUp size={14} /> Net Zakatable Assets</span>
                              <span className="font-black text-emerald-900">{fmt(zakatNet)}</span>
                           </div>
                        </div>

                        <div className="rounded-xl border border-emerald-200 bg-emerald-50/40 p-4 space-y-2">
                           <p className="text-[11px] font-black uppercase tracking-widest text-emerald-800">How It Is Calculated</p>
                           <p className="text-xs text-emerald-900">
                              Gross Zakatable Assets = Inventory + Receivables
                           </p>
                           <p className="text-xs text-emerald-900">
                              Net Zakatable Assets = Gross Zakatable Assets - Payables
                           </p>
                           <p className="text-xs text-emerald-900">
                              If Net Zakatable Assets {`>=`} Nisab, then Zakat Due = Net Zakatable Assets x {Math.round(zakatRate * 1000) / 10}%
                           </p>
                           <p className="text-[11px] text-emerald-700 font-medium pt-1">
                              Formula: ({fmt(zSource.inventoryValue)} + {fmt(zSource.receivables)} - {fmt(zSource.payables)}) x {Math.round(zakatRate * 1000) / 10}%
                           </p>
                        </div>

                        {zakatNet >= nisab ? (
                           <div className="p-6 rounded-2xl bg-emerald-600 text-white shadow-xl shadow-emerald-600/30 animate-in zoom-in-95">
                              <p className="text-[10px] font-black uppercase tracking-widest mb-1 opacity-80 text-emerald-100">Payable Zakat ({Math.round(zakatRate * 1000) / 10}%)</p>
                              <h3 className="text-3xl font-black">{fmt(zakatDue)}</h3>
                           ...
                           <div className="p-6 rounded-2xl bg-slate-100 text-slate-600 border border-slate-200">
                              <p className="text-sm font-bold flex items-center gap-2"><Info size={16} /> Not Eligible to Pay Zakat</p>
                              <p className="text-xs mt-2 opacity-80 font-medium">
                                 Net zakatable assets are below Nisab. Zakat is not obligatory at this time.
                              </p>
                           </div>
                        )}
                     </div>
